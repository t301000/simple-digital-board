...
/usr/bin/chromium-browser --incognito --start-fullscreen $url --user-data-dir=$user_data_dir --disable-web-security --test-type --disable-translate
# 運行 kiosk 模式
# 開啟時直接進入全螢幕模式，且無法利用 F11 跟 ESC 來離開全螢幕模式，可以用 ALT + F4 或 CTRL + W 來關閉。
# 在參數列加上 --kiosk 即可。
#/usr/bin/chromium-browser --kiosk $url --user-data-dir=$user_data_dir --disable-web-security --test-type --disable-translate
