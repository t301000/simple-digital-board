...
;(function(db) {
  ...
  function menuContentClickHandler(event) {
    ...
    if (srcElm.classList.contains('radio') && !srcElm.classList.contains('default')) {
      // 設為預設值，更新至 firestore
      db.doc(`channels/${iframe.dataset['id']}/actions/setDefault`).set({id: wrapper.dataset['id']});
    }
  }
  ...
  function updateDefaultMark() {
    ...
    radios.forEach(radio => {
      ...
      wrapper.dataset['id'] === id ? radio.classList.add('default') : radio.classList.remove('default');
    });
  }
  ...
  function setDefaultResource(obj) {
    obj ? localStorage.setItem('default', JSON.stringify(obj)) : localStorage.removeItem('default');
    updateDefaultMark();
  }
